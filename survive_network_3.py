...
NMD_RASTER_PATH = r"C:\Users\EricWahlsteen\OneDrive - Calluna AB\Skrivbordet\Python_test\Survive\nmd_amph.tif" #nmd_amph.tif or None 
USE_REAL_COST = True
...
# ===================== BLINK (eco-genetic with real LCP migrants) =====================

def simulate_network_demography_log_migrants(
    Nc0_vec: np.ndarray,
    Q: float,
    r: float,
    sigma_e_base: float,
    ...
    """
    As simulate_network_demography, but ALSO logs the *actual* immigrants
    arriving per patch per year (My_time[t, j]).

    Returns:
        Nc_time: (T_years+1, P)
        My_time: (T_years,   P) immigrants that arrived in year t to patch j
    """
    P = Nc0_vec.shape[0]
    Nc_t = Nc0_vec.astype(float).copy()
    Nc_time = np.zeros((T_years + 1, P), dtype=float)
    Nc_time[0, :] = Nc_t
    My_time = np.zeros((T_years, P), dtype=float)
    ...
    for t in range(int(T_years)):
        ...
        if landscape_mode == "infinite":
            My_vec = immigration_with_mainland(
                ...
                max_emigrants=max_emigrants,
                mainland_idx=mainland_idx,
                mainland_emigrants_per_year=float(mainland_emigrants_per_year or 0.0),
            )
        else:
            My_vec = immigration_from_connectivity(
                Nc_vec=Nc_t,
        ...
        # environmental variance multiplier for 'new pond' effect
        mult_t = disturbance_sigma_multiplier(
            t,
            start_factor=disturb_start,
            end_factor=disturb_end,
        ...
        climate = climate_modifiers(t)
        sigma_t = sigma_e_base * mult_t * climate.get("sigma_mult", 1.0)
        r_eff = (r + climate.get("r_delta", 0.0))
        if t == 0:
            r_eff *= INIT_PROP_ADULT  # founder breeder penalty
        K_eff = K * climate.get("K_mult", 1.0)

        Z = rng.standard_normal(P)
        if model == "theta-logistic":
            dens_term = r_eff * (1.0 - np.power(np.clip(Nc_t / max(K_eff, 1e-9), 0.0, np.inf), theta))
        else:
            dens_term = r_eff * (1.0 - Nc_t / max(K_eff, 1e-9))

        mu = dens_term - 0.5 * (sigma_t ** 2)

        # NEW: clip growth term to prevent exp overflow / underflow
        growth_term = np.clip(mu + sigma_t * Z, -10.0, 5.0)
        Nc_next = Nc_t * np.exp(growth_term) + My_vec

        # catastrophe and sanitation + NEW: cap to 5*K_eff
        Nc_next *= catastrophe_event(rng)
        Nc_next = np.where(~np.isfinite(Nc_next) | (Nc_next < 0), 0.0, Nc_next)
        Nc_next = np.minimum(Nc_next, 5.0 * max(K_eff, 1e-9))

        # enforce quasi-extinction floor (absorbing at Q)
        gone = (Nc_next <= Q)
        if np.any(gone):
            Nc_next[gone] = np.minimum(Nc_next[gone], Q)
    ...
    return Nc_time, My_time


def genetics_time_series_for_patch_from_arrivals(
    Nc_paths_patch: np.ndarray,        # shape (replicates, T_years+1)
    arrivals_patch: np.ndarray,        # shape (replicates, T_years)   immigrants per year to this patch
    rng_global: np.random.Generator,
    gen_time_years: float,
    H0: float,
    ne_ratio_start: float,
    ...
    """
    BLINK genetics for a single patch:
      - Drift each generation with Ne depending on BOTH time-since-foundation recovery
        and current abundance via a breeder-efficiency factor derived from Nc(t).
      - Migration per generation comes from the *real* number of immigrants that arrived
        during that generation window, taken from arrivals_patch.

    Returns (years, H_mean_series, H_lo, H_hi).
    """
    reps, T_plus1 = Nc_paths_patch.shape
    total_years = int(T_plus1 - 1)
    gens = int(np.floor(total_years / max(gen_time_years, 1e-9)))
    years = np.arange(0, gens + 1, dtype=float) * gen_time_years

    # initial allele frequency ~ 0.5; scale to H0
    p = np.full(reps, 0.5, dtype=float)
    scale = H0 / 0.5

    # record H at checkpoints
    ...
    for g in range(1, gens + 1):
        # calendar year at this generation boundary
        y = min(int(g * gen_time_years), total_years)

        Nc_now = Nc_paths_patch[:, y].astype(float)
        # Ne/Nc recovery with age of pond
        structural_start = ne_ratio_start * INIT_PROP_ADULT * (INIT_FEMALE_FRAC * (1.0 - INIT_FEMALE_FRAC)) * 4.0
        base_ratio = ne_ratio_over_time(
            t_years = g * gen_time_years,
            start_ratio = structural_start,
        ...
        abundance_factor = breeder_efficiency(Nc_now, half_sat=0.5 * K_scale, max_eff=1.0)
        ratio_t = base_ratio * abundance_factor

        # Ne: sanitize BEFORE casting; replace non-finite or <1 with 1
        Ne_g = ratio_t * Nc_now
        Ne_g = np.where(~np.isfinite(Ne_g) | (Ne_g < 1.0), 1.0, Ne_g)

        # migrants in this generation = sum of arrivals across the gen window
        y0 = max(0, int((g - 1) * gen_time_years))
        y1 = y
        m_gen = arrivals_patch[:, y0:y1].sum(axis=1)
        m_gen = np.where(~np.isfinite(m_gen) | (m_gen < 0), 0.0, m_gen)

        with np.errstate(divide='ignore', invalid='ignore'):
            m_frac = np.where(Nc_now > 0, np.clip(m_gen / Nc_now, 0.0, 1.0), 1.0)
        m_frac = np.where(~np.isfinite(m_frac), 0.0, m_frac)

        donor_p = rng.uniform(0.3, 0.7, size=reps)
        p = (1.0 - m_frac) * p + m_frac * donor_p

        # drift per replicate (fully guarded)
        new_p = np.empty_like(p)
        for i in range(reps):
            pi = p[i] if np.isfinite(p[i]) else 0.5
            twoNe = int(np.clip(2.0 * (Ne_g[i] if np.isfinite(Ne_g[i]) else 1.0), 2.0, 2000.0))
            new_p[i] = rng.binomial(n=twoNe, p=pi) / max(twoNe, 1)
        p = new_p

        H_mat[:, g] = 2.0 * p * (1.0 - p) * scale

    # summarize
    H_mean = H_mat.mean(axis=0)
    # bootstrap CI across reps at each generation step
    def _boot(vec):
        # simple bootstrap CI
        rng_b = np.random.default_rng(rng.integers(0, 2**32 - 1))
        means = [rng_b.choice(vec, size=len(vec), replace=True).mean() for _ in range(500)]
        return float(np.percentile(means, 2.5)), float(np.percentile(means, 97.5))

    H_lo = np.zeros_like(H_mean)
    H_hi = np.zeros_like(H_mean)
    for k in range(H_mat.shape[1]):
        lo, hi = _boot(H_mat[:, k])
        H_lo[k] = lo
        H_hi[k] = hi

    return years, H_mean, H_lo, H_hi


def run_network_replicates_blink(
    patches: list[Patch],
    Pmat: np.ndarray,
    r: float,
    sigma_e: float,
    ...
    """
    Like run_network_replicates, but returns:
      - Nc_paths_all: (replicates, T_years+1, P)
      - arrivals_all: (replicates, T_years,   P)
      - survived_mask: (replicates, P)
    """
    P = len(patches)
    Nc0_vec = np.array([p.Nc0 for p in patches], dtype=float)

    Nc_paths_all = np.zeros((replicates, T_years + 1, P), dtype=float)
    arrivals_all = np.zeros((replicates, T_years, P), dtype=float)
    survived_mask = np.zeros((replicates, P), dtype=float)

    for rep in range(replicates):
        rng = np.random.default_rng(rng_global.integers(0, 2**32 - 1))

        Nc_time, My_time = simulate_network_demography_log_migrants(
            Nc0_vec=Nc0_vec,
            Q=Q,
            r=r,
            sigma_e_base=sigma_e,
        ...
        Nc_paths_all[rep, :, :] = Nc_time
        arrivals_all[rep, :, :] = My_time
        survived_mask[rep, :] = (Nc_time[-1, :] > Q).astype(float)

    return Nc_paths_all, arrivals_all, survived_mask


def plot_heterozygosity_over_time_blink(
    Nc_paths_all: np.ndarray,      # (replicates, T_years+1, P)
    arrivals_all: np.ndarray,      # (replicates, T_years,   P)
    patches: list[Patch],
    results_dir: str,
):
    """
    Plot BLINK heterozygosity for each patch using *actual arrivals* per year
    from the demography runs.
    """
    reps, T_plus1, P = Nc_paths_all.shape
    rng_master = np.random.default_rng(RANDOM_SEED)
    ...
    any_years = None
    for j, patch in enumerate(patches):
        years, H_mean, H_lo, H_hi = genetics_time_series_for_patch_from_arrivals(
            Nc_paths_patch=Nc_paths_all[:, :, j],
            arrivals_patch=arrivals_all[:, :, j],
            rng_global=rng_master,
            gen_time_years=GEN_TIME_FOR_GENETICS,
            H0=H0,
            ne_ratio_start=NE_RATIO_START,
        ...
        any_years = years
        plt.plot(years, H_mean, label=patch.name, linewidth=1.8)
        plt.fill_between(years, H_lo, H_hi, alpha=0.15)

    # threshold
    H_min = H_MIN_FRAC * H0
    plt.axhline(H_min, linestyle="--", linewidth=1.2,
                label=f"H threshold ({H_MIN_FRAC:.2f}×H₀)")

    plt.xlabel("År")
    plt.ylabel("Heterozygositet H(t)")
    plt.title("BLINK (nätverk): Genetik med verkliga LCP-invandrare + demografisk kaos (C)")
    plt.legend(fontsize=8, ncol=2)
    plt.tight_layout()
    out_path = os.path.join(results_dir, "mod_Blink_network_real_migrants.png")
    plt.savefig(out_path, dpi=150)
    plt.close()
    print(f"Saved BLINK-network figure: {out_path}")

def summarize_connectivity_global(Pmat: np.ndarray) -> str:
    v = Pmat[np.isfinite(Pmat)]
    v = v[v > 0]
    if v.size == 0:
        return "No positive links."
...
def main_network_blink():
    """
    BLINK network run:
      - load patches
      - build Euclid + cost matrices (least-cost if raster provided)
      - build Pmat
      - (optional) add mainland donor for LANDSCAPE_MODE == "infinite"
      - run demography while LOGGING immigrants per year
      - run genetics using *actual arrivals* per generation window
      - save arrays + BLINK figure
    """
    rng_master = np.random.default_rng(RANDOM_SEED)
    results_dir = make_results_dir(RESULTS_BASE)
    print(f"[BLINK] Results directory: {results_dir}")

    # --- 1) Load patches (FIX: actually load them) ---
    patch_file = os.environ.get("PATCH_FILE", PATCH_FILE)
    if not os.path.exists(patch_file):
        raise FileNotFoundError(f"Patch file not found: {patch_file}")
    patches = load_patches_from_file(patch_file)
    P = len(patches)
    print(f"[BLINK] Loaded {P} patches from: {patch_file}")

    # --- 2) Euclidean distances ---
    D_euclid = build_euclid_matrix(patches)

    # --- 3) Cost distances (least-cost if raster available) ---
    friction_tif = os.environ.get("NMD_RASTER_PATH", NMD_RASTER_PATH)
    friction_win = transform_win = patch_rc = None
    if friction_tif and os.path.exists(friction_tif):
        try:
            C_cost, friction_win, transform_win, patch_rc = build_cost_matrix_least_cost(
                patches=patches,
                friction_tif_path=friction_tif,
                buffer_dist=MAX_DISPERSAL_DIST * 4.0,
                barrier_shp_path=(BARRIER_SHP if BARRIER_SHP and BARRIER_SHP != "None" else None),
                barrier_mode="multiply",
                barrier_value=50.0,
                line_buffer_m=5.0,
                attr_mult_field="cost_mult",
            )
            print("[BLINK] Using least-cost distances from raster.")
        except Exception as e:
            print(f"[BLINK] Cost-distance failed ({e}); falling back to Euclid-based dummy cost.")
            C_cost = build_dummy_cost_matrix_from_distance(D_euclid, road_multiplier=1.0)
    else:
        print("[BLINK] No friction raster provided – using distance-decay on Euclidean distances.")
        C_cost = build_dummy_cost_matrix_from_distance(D_euclid, road_multiplier=1.0)

    # --- 4) Connectivity P matrix ---
    beta_cost = calibrate_beta_from_costs(C_cost, target_P=0.25, quantile=0.10)
    Pmat = compute_pairwise_matrices(
        patches=patches,
        C_cost_matrix=C_cost,
    ...
    conn_str = summarize_connectivity_global(Pmat)
    print("[CONNECTIVITY]", conn_str)
    lost = int(np.sum((np.isfinite(C_cost)) & (C_cost > 0) & (Pmat == 0.0)))
    print(f"[CONNECTIVITY] pairs with P=0 after barrier: {lost}")

    # --- 5) Mainland donor (if requested) ---
    mainland_idx = None
    mainland_emigrants = None
    if LANDSCAPE_MODE == "infinite":
        patches, Pmat, mainland_idx = add_mainland_donor(
        ...
        mainland_emigrants = INFINITE_SOURCE_DISPERSERS_PER_YEAR
        print(f"[BLINK] Added mainland donor at index {mainland_idx} with {mainland_emigrants}/yr.")

    # --- 6) Run demography with immigrant logging (BLINK) ---
    r = R_GRID[0] if isinstance(R_GRID, (list, tuple, np.ndarray)) else float(R_GRID)
    sigma_e = SIGMA_E_GRID[0] if isinstance(SIGMA_E_GRID, (list, tuple, np.ndarray)) else float(SIGMA_E_GRID)

    Nc_paths_all, arrivals_all, survived_mask = run_network_replicates_blink(
        patches=patches,
        Pmat=Pmat,
        r=r,
        sigma_e=sigma_e,
        ...
        model=DENSITY_MODEL,
        theta=THETA,
        landscape_mode=("infinite" if LANDSCAPE_MODE == "infinite" else "finite"),
        mainland_idx=mainland_idx,
        mainland_emigrants_per_year=mainland_emigrants,
    )

    # --- 7) Save arrays + manifest ---
    np.save(os.path.join(results_dir, "Nc_paths_all.npy"), Nc_paths_all)
    np.save(os.path.join(results_dir, "arrivals_all.npy"), arrivals_all)
    derived_bits = {
        "beta_cost": float(beta_cost),
        "connectivity_summary": str(conn_str),
        "pairs_with_P0_after_barrier": lost,
        "cost_mode": ("least_cost" if (friction_tif and os.path.exists(friction_tif)) else "euclid_dummy"),
        "barrier_file": (BARRIER_SHP if BARRIER_SHP and BARRIER_SHP != "None" else None),
        "tile_path": (friction_tif if (friction_tif and os.path.exists(friction_tif)) else None),
    }
    write_run_manifest(
        results_dir=results_dir,
        cfg_module=cfg,
    ...
    # --- 8) Plot BLINK genetics using *actual* arrivals ---
    plot_heterozygosity_over_time_blink(
        Nc_paths_all=Nc_paths_all,
    ...
    print("[BLINK] Done. Arrays + figure saved in:", results_dir)
